...
export default function InvoiceForm({ initialData, onSave, isUpdating = false }: InvoiceFormProps) {
  ...
  return (
    <form onSubmit={form.handleSubmit(onSubmit)}>
      <Card>
        <CardHeader>
          <CardTitle>{isUpdating ? 'Edit Invoice' : 'Verify Extracted Data'}</CardTitle>
        ...
        <CardContent className="space-y-6">
          ...
          <div className="space-y-2">
            ...
            <div>
              ...
              {form.formState.errors.vendor?.name && <p className="text-sm text-red-500">{form.formState.errors.vendor.name.message}</p>}
            </div>
            <div>
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <Input id="invoice.number" {...form.register('invoice.number')} />
                 {form.formState.errors.invoice?.number && <p className="text-sm text-red-500">{form.formState.errors.invoice.number.message}</p>}
              </div>
              <div>
                <Label htmlFor="invoice.date">Invoice Date</Label>
                <Input id="invoice.date" {...form.register('invoice.date')} />
                 {form.formState.errors.invoice?.date && <p className="text-sm text-red-500">{form.formState.errors.invoice.date.message}</p>}
              </div>
            </div>
          </div>
              ...
              <TableBody>
                {fields.map((field, index) => (
                  <TableRow key={field.id}>
                    <TableCell>
                      <Input {...form.register(`invoice.lineItems.${index}.description`)} />
                    </TableCell>
                    <TableCell>
                      <Input type="number"
                      step="any" 
                    ...
                    <TableCell>
                      <Input type="number"
                      step="any" 
                      {...form.register(`invoice.lineItems.${index}.unitPrice`,{valueAsNumber:true})} />
                    </TableCell>
                    <TableCell>
